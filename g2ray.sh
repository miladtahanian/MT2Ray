...
draw_logo() {
    ...
    echo " ╚██████╔╝███████╗██║  ██║██║  ██║   ██║   "
    echo "  ╚═════╝ ╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝   ╚═╝   "
    echo -e "${NC}${WHITE}  MT2Ray Panel | Made By Milad Tahanian${NC}\n"
}
...
generate_link() {
    ...
    PUBLIC_IP=$(curl -s --max-time 6 https://api.ipify.org 2>/dev/null || \
                ...
                echo "94.130.50.12")
    echo "vless://${UUID}@${PUBLIC_IP}:${XRAY_PORT}?encryption=none&security=tls&sni=${DOMAIN}&fp=chrome&alpn=h2&insecure=1&allowInsecure=1&type=xhttp&host=${DOMAIN}&path=%2F&mode=packet-up#MTServer"
}
...
if [ ! -f "$CONFIG_FILE" ]; then
    clear; draw_logo
    echo -e "  ${WHITE}Welcome to MT2Ray Setup!${NC}"
...
while true; do
    ...
    case $_choice in
        ...
        1)
            ...
            clear; draw_logo
            echo -e "  ${GREEN}Scan to Connect (MT2Ray):${NC}\n"
        ...
        8)
            clear; draw_logo
            echo -e "${GREEN}📡 MT2Ray Data Usage (All Sessions)${NC}\n"
